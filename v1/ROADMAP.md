...
- **Per-trigger learning loop (2026-05-17, Phase 1)** — the agent captures user corrections via new `learning_*` MCP tools: positive/negative examples per trigger (`email_triage`, `morning_brief`, `weekly_review`) live in a new `trigger_examples` SQLite table. At call time, `scheduler/trigger_prompts.render_examples_block` prepends up to 3 positive + 3 negative recent examples to each trigger's prompt. Soft-delete via web UI at `/learning` or chat command. Brief / weekly-review feedback flow: agent fetches the last fire's assembled prompt via `learning_get_last_trigger_fire` before recording the correction, so `input_payload` reflects what the trigger actually saw.
- **Learning loop Phase 2 — delivery_watch + expected_arrivals LLM gates (2026-05-17)** — added a shared `_haiku_decision_gate` helper in `scheduler/triggers.py` that takes a candidate email, prepends the trigger's examples block, asks Haiku a yes/no question, and parses VERDICT/REASON plus optional extracted fields (CARRIER, TRACKING, TRACKING_URL for delivery_watch). For `_fire_delivery_watch`: the legacy carrier+keyword string match becomes the cheap pre-filter, and the gate is the precision pass — catches marketing emails that mention "out for delivery" while learning from user negatives. The gate's extracted tracking info is preferred over the regex fallback when populated. For `_fire_expected_arrivals`: `_find_expected_arrival_candidates` now returns up to 5 candidate emails (was: bool "exists"), and each is gated by Haiku asking "is this the email the user has been waiting for about <watch_name>?" — biased toward NO so weak matches keep the gap-detect ping alive. All 5 triggers now learn from chat corrections.
- **Three-tier reply: text / single-emoji / skip (2026-05-17)** — the agent now matches reply length to message intent. Full text for info asks and action-with-follow-up; a single emoji (👍 ❤️ ✅ 🙏 🌙 👋) for pure acks with sentiment; no reply at all for closing-shaped messages like "ok" / "got it" / tapbacks. Implementation is entirely a `personality.md` rule — the relays already supported empty-reply skips and 1-char sends. Native Telegram / Discord / Slack reactions (`setMessageReaction`, `add_reaction`, `reactions.add`) tracked as a possible v2 polish; iMessage tapbacks aren't reachable via AppleScript without private-framework injection so the text-emoji path is the right primary on the principal's main transport.
