    ...
    TransportDef(
        ...
        fields=[
            TransportField(
                key="IMESSAGE_MODE",
                label="Mode",
                help="`self` = text yourself from your iPhone (note-to-self thread, default). `contact` = listen to one specific other person. `dedicated` = the agent has its OWN Apple ID signed in to Messages.app — replies render as inbound gray bubbles. See SETUP.md → 'Advanced: dedicated agent identity' for the account setup; it has a policy caveat worth reading first.",
                select_options=["self", "contact", "dedicated"],
                default="self",
            ),
            TransportField(
                key="TARGET_PHONE_NUMBER",
                label="Target phone",
                help="In self mode: your own number (+15551234567). In contact mode: the other person's number or Apple-ID email. In dedicated mode: leave blank (use IMESSAGE_USER_HANDLE + IMESSAGE_AGENT_APPLE_ID below instead).",
            ...
            TransportField(
                ...
                help="self mode only: extra handles to watch alongside TARGET_PHONE_NUMBER, e.g. your Apple ID email. Comma-separated.",
                optional=True,
                secret=True,
            ),
            TransportField(
                key="IMESSAGE_USER_HANDLE",
                label="User handle (dedicated mode)",
                help="dedicated mode only: YOUR primary Apple ID handle (phone +15551234567 or email). The agent reads incoming messages from this handle.",
                placeholder="+15551234567",
                secret=True,
                optional=True,
            ),
            TransportField(
                key="IMESSAGE_AGENT_APPLE_ID",
                label="Agent Apple ID (dedicated mode)",
                help="dedicated mode only: the email of the AGENT's Apple ID, which must be signed in to Messages.app. Used to scope AppleScript sends to the right iMessage service. Run `python -m relay.imessage_relay --list-services` to verify.",
